...
int DGifGetLineByte(GifFileType *GifFile, GifPixelType *Line, int LineLen){
	...
	PA_OutputText(1, GifBits*10, (++test), "ok ");
	PA_OutputText(1, 3, 0, "%d     ", GifBits);
...
{
	PA_OutputText(1, 0, (++test), "ok ");
	PA_OutputText(1, 10 + GifBits*10, 0, "%d", GifBits);
	...
	DGifGetRecordType(GifFile, &RecordType);
		PA_OutputText(1, 0, 22, "%dvs%d", RecordType, IMAGE_DESC_RECORD_TYPE);
	switch (RecordType) {
	case IMAGE_DESC_RECORD_TYPE:
		...
		PA_OutputText(1, 0, 23, "%dx%d", Width, Height);
