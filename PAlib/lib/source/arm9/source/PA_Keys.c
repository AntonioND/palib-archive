...
void PA_UpdatePad(void) {
   ExPad = CompletePad;
   CompletePad = ~(PA_BUTTONS + (((PA_IPC_compat->buttons)&3) << 10));
...
u8 temp = ((~PA_IPC_compat->buttons) >> 6) & 1;

	Stylus.Pressure = (((PA_IPC_compat->touchXpx * PA_IPC_compat->touchZ2) >> 6) / PA_IPC_compat->touchZ1) - (PA_IPC_compat->touchXpx >> 6);
	...
	if (Stylus.Held) { // On en met à jour que si on touche l'écran, histoire de pas avoir un truc faussé
		Stylus.altX =  ((PA_IPC_compat->touchX - 0x0113) / 14);
		Stylus.altY =  ((PA_IPC_compat->touchY - 0x00E0) / 19);
		...
		if(Stylus.Newpress){
			...
			Stylus.Vx = PA_IPC_compat->touchXpx - Stylus.X;
			Stylus.Vy = PA_IPC_compat->touchYpx - Stylus.Y;					
...
//			Stylus.Y = IPC->touchYpx;
		...
		while (PA_IPC_compat->mailBusy);		
		Stylus.X =  PA_IPC_compat->touchXpx;
		Stylus.Y =  PA_IPC_compat->touchYpx;			
