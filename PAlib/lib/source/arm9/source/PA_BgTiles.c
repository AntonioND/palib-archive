...
void PA_InitBg(u8 screen, u8 bg_select, u8 bg_size, u8 wraparound, u8 color_mode) {	
	...
	if(!screen){
		...
		//BGCTRL[bg_select] = BG_MAP_BASE(PA_BgInfo[screen][bg_select].mapchar) | BG_TILE_BASE(PA_BgInfo[screen][bg_select].TileSetChar) | bg_size;
		...
		BGCTRL_SUB[bg_select] = bg_select |		// Priority
			...
			BG_TILE_BASE(PA_BgInfo[screen][bg_select].TileSetChar) | // Tile base
			(color_mode << 7);				// Color mode?
		//BGCTRL_SUB[bg_select] = BG_MAP_BASE(PA_BgInfo[screen][bg_select].mapchar) | BG_TILE_BASE(PA_BgInfo[screen][bg_select].TileSetChar) | bg_size;
	}
...
void PA_DeleteMap(u8 screen, u8 bg_select) {
	if (PA_BgInfo[screen][bg_select].mapsize) { // Si y'a un truc, on efface
		...
		dmaFillWords(0, (void*)CharBaseBlock(screen, charset), (blocksize << 10)*2);
