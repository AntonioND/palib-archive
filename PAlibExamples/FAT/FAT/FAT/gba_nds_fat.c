...
bool FAT_FreeFiles (void)
{
	...
	// Clear card status
	disc_ClearStatus();
  disc_Shutdown();  // ALekmaul : no status because it is cleared before
...
bool FAT_rename (const char *oldName, const char *newName)
{
	DIR_ENT oldDir;
	DIR_ENT newDir;
	u32 tmpwrkDirCluster;
	int tmpwrkDirSector;
	int tmpwrkDirOffset;
	
	oldDir = FAT_DirEntFromPath (oldName);
	
	if(oldDir.name[0] == FILE_FREE) // old filename doesn't exist
		return false;
	
	newDir = FAT_DirEntFromPath (newName);
	
	if(newDir.name[0] != FILE_FREE) // new filename exists already
		return false;
	
	newDir = FAT_DirEntFromPath (oldName);	// refresh
	
	tmpwrkDirCluster = wrkDirCluster;
	tmpwrkDirSector = wrkDirSector;
	tmpwrkDirOffset = wrkDirOffset;
	
	if(FAT_AddDirEntry (newName, newDir)) // add new entry using start cluster of old directory, if fails, write back old entry
	{
		disc_ReadSector ( (tmpwrkDirCluster == FAT16_ROOT_DIR_CLUSTER ? filesysRootDir : FAT_ClustToSect(tmpwrkDirCluster)) + tmpwrkDirSector , globalBuffer);
		((DIR_ENT*)globalBuffer)[tmpwrkDirOffset].name[0] = FILE_FREE;
		disc_WriteSector ( (tmpwrkDirCluster == FAT16_ROOT_DIR_CLUSTER ? filesysRootDir : FAT_ClustToSect(tmpwrkDirCluster)) + tmpwrkDirSector , globalBuffer);
		
		return disc_CacheFlush();
	}
...
char *FAT_getCurDir() {
  ...
  dir = FAT_DirEntFromPath ("./");

  if (((dir.attrib & ATTRIB_DIR) == ATTRIB_DIR) && (dir.name[0] != FILE_FREE)) {
    // Change directory
    strcpy(strCurrentDir,dir.name);
  }

  FAT_chdir(strCurrentDir);
}

char *FAT_tryingInterface() {
  switch (whichInterface) {
    case IS_NONE :
      strcpy(strInterface, "None");
      break;
    case IS_MPCF :
      strcpy(strInterface, "GBA Movie Player CF");
      break;
    case IS_M3CF :
      strcpy(strInterface, "M3 perfect CF");
      break;
    case IS_M3SD :
      strcpy(strInterface, "M3 perfect SD");
      break;
    case IS_SCCF :
    ...
    case IS_SCSD :
      ...
      break;
    case IS_EFA2 :
      strcpy(strInterface, "EFA2");
      break;
    case IS_FCSR :
      strcpy(strInterface, "GBA Flash Card");
      break;
    case IS_NMMC :
      strcpy(strInterface, "MK2/MK3");
      break;
    case IS_EZSD :
      strcpy(strInterface, "EZ-Flash4 SD");
      break;
    case IS_NJSD :
      strcpy(strInterface, "NinjaSD");
      break;
    case IS_MMCF :
      strcpy(strInterface, "Max Media CF Support");
      break;
    default :
      strcpy(strInterface, "None");
      break;
