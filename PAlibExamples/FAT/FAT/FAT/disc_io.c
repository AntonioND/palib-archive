...
#ifdef SUPPORT_MMCF
	// check if we have a Max Media CF plugged in
	active_interface = MMCF_GetInterface() ;
	if (active_interface->fn_StartUp())
	{		
		whichInterface = IS_MMCF;
		return true ;
	} ;
#endif
...
#ifdef SUPPORT_NJSD
	// check if we have a Ninja Flashed plugged in
	active_interface = NJSD_GetInterface() ;
	if (active_interface->fn_StartUp())
	{
		// set NinjaSD as default IO
		whichInterface = IS_NJSD;
		return true ;
	} ;
#endif
